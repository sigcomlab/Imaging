...
tx_x (10:12) = []; 
tx_y (10:12) = [];
tx_y = permute(tx_y,[4,3,2,1]) + randn(1,1,9)*0.000001; 

% rx_x = [0 0 0 0];
% rx_y = [0.     0.0019 0.0039 0.0058];  % [0 0.0019 0.0038 0.0057];
rx_y = permute(rx_y,[4,3,1,2]) + randn(1,1,1,16)*0.000001;
...
%%
load('pliers_calibrated.mat');  % [12, 53, 403, 256] [TX*RX Vstep Hstep N]
rawDataFFT = fft(s_full,N0,4);
clear rawDataCal
...
sarData = sarData (:,[1:9],:);
sarData = permute(sarData,[1,4,5,2,3]); %  403    53     2     4
